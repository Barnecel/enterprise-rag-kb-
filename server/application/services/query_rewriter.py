...
class QueryRewriter:
    ...
    SYNONYM_RULES = [
        ...
        # Q10 badcase：规则句被元素/父窗口边界切断，查询词分布两侧——变体需同时含两侧词项
        (r'(当场处罚|简易程序).{0,14}(条件|适用|情形)', '当场处罚 违法事实确凿 一名以上人民警察 五百元以下'),
        # Q7 badcase：时限内容跨父窗口，直填时限词项强匹配
        (r'传唤.{0,20}时限|(几小时|多久|时限).{0,20}(传唤|继续盘问)|(区别|对比).{0,30}(传唤|盘问)', '继续盘问 12小时 24小时 48小时 传唤 询问查证 8小时'),
    ]
    """查询改写器"""
