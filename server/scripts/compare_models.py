...
def main():
    ...
    if JUDGE.get("api_base"):
        ...
        for i in range(len(summary["A"])):
            ...
            verdict = call_judge(JUDGE, ra["question"],
                                 next(q["reference"] for q in bank if q["id"] == ra["qid"]),
                                 next(q["trap"] for q in bank if q["id"] == ra["qid"]),
                                 pair[0][0]["answer"], pair[1][0]["answer"])
        ...
        print("未配置JUDGE，跳过自动打分。把两份 compare_*.json 贴给裁判人工/LLM盲评。")
